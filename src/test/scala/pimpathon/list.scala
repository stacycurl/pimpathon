...
class ListTest extends PSpec {
  "uncons" in on(nil[Int], List(1, 2, 3))
    .calling(_.uncons("empty", l ⇒ s"size: ${l.size}")).produces("empty", "size: 3")

  "unconsC" in on(nil[Int], List(1, 2, 3))
    .calling(_.unconsC("empty", h ⇒ t ⇒ s"head: $h, tail: $t")).produces("empty", "head: 1, tail: List(2, 3)")

  "unsnocC" in on(nil[Int], List(1, 2, 3))
    .calling(_.unsnocC("empty", i ⇒ l ⇒ s"init: $i, last: $l")).produces("empty", "init: List(1, 2), last: 3")

  "emptyTo" in on(nil[Int], List(1, 2, 3)).calling(_.emptyTo(List(1))).produces(List(1), List(1, 2, 3))

  "calcIfNonEmpty" in on(nil[Int], List(1, 2, 3))
    .calling(_.calcIfNonEmpty(_.reverse)).produces(None, Some(List(3, 2, 1)))

  "mapIfNonEmpty" in on(nil[Int], List(1, 2, 3))
  ...
  "zipWith" in List(2, 0).zipWith(List(3))(lr ⇒ lr._1 * lr._2) ≡ List(6)

  "countWithSize" in on(nil[Int], List(0), List(1), List(0, 1))
  ...
  "countBy" in List("foo", "bard", "food", "barb", "foody", "barby").countBy(_.length) ≡
  ...
  "headTailOption" in on(nil[Int], List(1), List(1, 2), List(1, 2, 3))
    .calling(_.headTailOption).produces(None, Some((1, Nil)), Some((1, List(2))), Some((1, List(2, 3))))

  "initLastOption" in on(nil[Int], List(1), List(1, 2), List(1, 2, 3))
    .calling(_.initLastOption).produces(None, Some((Nil, 1)), Some((List(1), 2)), Some((List(1, 2), 3)))

  "initOption" in on(nil[Int], List(1), List(1, 2), List(1, 2, 3))
    .calling(_.initOption).produces(None, Some(Nil), Some(List(1)), Some(List(1, 2)))

  "const" in on(nil[Int], List('a', 'b', 'c')).calling(_.const(1)).produces(nil[Int], List(1, 1, 1))
  ...
  "batchWhile" in List("foo", "food", "bar", "bare", "barf")
    .batchWhile(_.mkString("").distinct.length <= 4) ≡ List(List("foo", "food"), List("bar", "bare"), List("barf"))

  "prefixPadTo" in List(1, 2, 3).prefixPadTo(6, 0) ≡ List(0, 0, 0, 1, 2, 3)
  ...
  "amass" in List(1, 2, 3, 4).amass { case i if i % 2 == 0 ⇒ List(i, -i) } ≡ List(2, -2, 4, -4)

  "interleave" in List(1, 2, 3).interleave(List(10, 20)) ≡ List(1, 10, 2, 20, 3)

  "interleaveWith" in
    List(1, 2, 3).interleaveWith(List("ten", "twenty"))(_.valueOr(_.toString)) ≡ List("1", "ten", "2", "twenty", "3")

  "cartesianProduct" in List(List(1, 2), List(10, 20), List(100, 200)).cartesianProduct ≡
  ...
  "onlyOption" in on(nil[Int], List(1, 2), List(1)).calling(_.onlyOption).produces(None, None, Some(1))
  ...
  "toMultiMap" in on(List((1, 10), (1, 11), (2, 20), (2, 21)))
  ...
  "shuffle" in List.range(1, 10).shuffle().sorted ≡ List.range(1, 10)
