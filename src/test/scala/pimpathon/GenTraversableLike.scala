...
class GenTraversableLikeSpec extends PSpec {
  ...
  "onlyOrThrow" in {
    on(
      Set.empty[Int],  Set(1, 2),
      List.empty[Int], List(1, 2)
    ).calling(_.onlyOrThrow(s ⇒ exception(s"${s.size} elements"))).throws(
      "0 elements", "2 elements", "0 elements", "2 elements"
    )
    
    on(List(1), Set(2)).calling(_.onlyOrThrow(_ ⇒ new Exception())).produces(1, 2)
  }

  "onlyEither" in on(
    Set.empty[Int],  Set(1, 2),  Set(1),
    List.empty[Int], List(1, 2), List(1)
  ).calling(_.onlyEither).produces(
    Left(Set.empty[Int]),  Left(Set(1, 2)),  Right(1),
    Left(List.empty[Int]), Left(List(1, 2)), Right(1)
  )
  
  "onlyOption" in on(
    Set.empty[Int], Set(1, 2), Set(1),
    List.empty[Int], List(1, 2), List(1)
  ).calling(_.onlyOption).produces(
    None, None, Some(1),
    None, None, Some(1)
  )
  ...
  "partitionEithers" - {
    "List" in {
      gtl(Left(1), Right("abc"), Right("def"), Left(2)).partitionEithers[List] ≡ (List(1, 2), List("abc", "def"))
    }
